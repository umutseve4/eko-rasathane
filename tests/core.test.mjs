...
test('invalid and non-canonical routes are rejected',()=>{for(const route of ['#/sinif/9','#/sinif/3/fazla','#/ders/x/fazla','#/ders/%E0%A4%A'])assert.equal(parseRoute(route).name,'notFound')});
...
test('migration preserves production legacy data',()=>{const s=migrateState(null,null,{evidence:[{id:1}],studio:{step:2},notes:{old:'metin'}});assert.equal(s.version,2);assert.deepEqual(s.evidence,[{id:1}]);assert.deepEqual(s.legacyStudio,{step:2});assert.equal(s.notes.old,'metin')});
test('migration repairs incomplete v2 state',()=>{const s=migrateState(null,{version:2});assert.deepEqual(s.completedTopics,[]);assert.deepEqual(s.quizResults,{});assert.deepEqual(s.notes,{})});
